...
1. **Acoustic Profiling & Data Integrity:**
   ...
   * Discovered that the official validation set is a known-speaker benchmark (20/20 speaker overlap in training). To guarantee an honest out-of-speaker evaluation, we curated a derived training set sampled strictly from the **103 exclusive training speakers** (0 speaker overlap: `len(train_speakers & val_speakers) == 0`).
...
3. **Cloud Training Convergence (EXP-007 on Kaggle Tesla T4):**
   ...
   * Cross-entropy loss decreased by **-45.60% relative** (from 3.0641 to 1.6670, reaching a minimum of 1.6160 at step 170) with stable gradients (mean norm 0.1496) and flat 4,875.8 MB VRAM allocation.
